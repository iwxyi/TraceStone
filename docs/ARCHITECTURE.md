...
| 离线可用 | 无网络时编辑、搜索、回顾等本地能力仍可用 |
| 模块隔离 | 日记、洞察、塑石、陪伴、搜索、关系、设置独立演进 |

## 分层结构

| 层 | 职责 | 例子 |
| --- | --- | --- |
| Presentation | 页面、组件、交互状态 | 日记编辑页、塑石列表页 |
| Application | 用例编排、权限检查、状态流转 | 保存日记、生成洞察、采纳塑石 |
...
| AI Insight | DiaryEntry、Memory Context | 修复文本、洞察、摘要、标签、塑石建议 |
| Shaping Stone | AI 建议、用户编辑 | StoneTask、打卡记录、完成率 |
| Companion | 用户问题、检索到的记忆 | 引用历史的回答、引导式追问 |
| Review | 周/月/年日记、塑石记录 | 周报、月报、年报、成长曲线 |
