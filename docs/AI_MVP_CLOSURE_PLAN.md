# 拾年·Shinen AI MVP 收束计划

产品主题已调整为“记录与成长”。AI 输出必须保持轻松、具体、不施压：不把成长做成评分，不把建议做成任务，不做打卡、连续天数或完成率。
...
2. 系统生成摘要、分段、基础向量、历史上下文和今日分析。
3. 今日分析以当前日记为主，引用历史时必须来自已检索到的来源。
...
| 今日分析生成 | 输出读后感、情绪、关键词、成长线索、结构化 claims | 内容以今日日记为主，历史只作辅助 |
...
- 用户反馈“不准确”会将当前 job 标记为可继续的重生成流程。
